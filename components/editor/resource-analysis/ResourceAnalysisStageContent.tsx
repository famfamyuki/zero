...
export function ResourceAnalysisStageContent({ state, isRefreshing, lang, notice, onRetry, onOpenValidation, onLocate }: ResourceAnalysisStageContentProps) {
  ...
  const hotspotWhy = (kind: ResourceAnalysisHotspot['kind']) => ({
    dependency_depth: lang === 'ja' ? '静的な依存関係で最も深い位置です。' : 'This is the deepest position in the static dependency structure.',
    context_fan_in: lang === 'ja' ? '直接参照するコンテキスト入力が最も集中しています。' : 'This target has the highest number of direct context inputs.',
    tool_binding_concentration: lang === 'ja' ? '静的なToolバインディングが最も集中しています。' : 'This target has the highest concentration of static Tool bindings.',
  })[kind];
  const targetLabel = (target: ResourceAnalysisTarget) => target.type === 'crew' ? 'Crew' : `${target.type} · ${target.id}`;
  ...
  {!isRefreshing && result ? <>
    ...
    <section className={sectionClass}><h3 className="text-sm font-bold text-white">{copy.resourceAnalysisHotspots}</h3>{result.hotspots.length ? <ul className="mt-3 space-y-3">{result.hotspots.map((item, index) => <li key={`${item.kind}-${item.target.type}-${'id' in item.target ? item.target.id : 'crew'}-${index}`} className="rounded-xl bg-slate-950/80 p-3"><span className="rounded-full border border-violet-800 px-2 py-0.5 text-[10px] font-bold text-violet-200">Hotspot</span><p className="mt-2 text-xs font-bold text-slate-100">{hotspotLabel(item.kind)}</p><p className="mt-2 text-[10px] font-bold uppercase text-slate-500">{lang === 'ja' ? '場所' : 'Where'}</p><p className="text-[11px] text-slate-300">{targetLabel(item.target)}</p><p className="mt-2 text-[10px] font-bold uppercase text-slate-500">{lang === 'ja' ? '理由' : 'Why'}</p><p className="text-[11px] leading-relaxed text-slate-400">{hotspotWhy(item.kind)}</p><p className="mt-2 text-[11px] text-slate-400">{lang === 'ja' ? '決定論的な値' : 'Deterministic value'}: <span className="font-bold text-violet-200">{item.value}</span></p><button type="button" onClick={() => locate(item)} className={`mt-2 ${actionClass}`}>{copy.resourceAnalysisLocate}</button></li>)}</ul> : <p className="mt-2 text-xs leading-relaxed text-slate-400">{copy.resourceAnalysisNoHotspots}</p>}</section>
