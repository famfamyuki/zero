...
import { CheckSquare, ArrowRightCircle, Settings, Trash2 } from 'lucide-react';
...
export const TaskNode = memo(({ id, data, selected }: NodeProps<any>) => {
  ...
  return (
    ...
    >
      ...
      {selected && (
        <div className="absolute -top-11 left-1/2 -translate-x-1/2 z-50 flex items-center gap-1 bg-slate-900 border border-emerald-500/80 p-1 rounded-full shadow-2xl animate-in zoom-in-95 duration-150 shrink-0">
          <button
            onClick={handleEditClick}
            className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-emerald-600 hover:bg-emerald-500 text-white text-[11px] font-semibold transition"
          >
      ...
      {/* Header Bar */}
      <div className="bg-gradient-to-r from-emerald-950 via-slate-900 to-emerald-900/80 px-4 py-2.5 border-b border-emerald-900/40 flex items-center justify-between rounded-t-xl">
