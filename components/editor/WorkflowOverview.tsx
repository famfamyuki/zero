...
export function WorkflowOverview(props: Props) {
  ...
  return <main className="min-h-0 flex-1 overflow-y-auto" aria-labelledby="overview-heading">
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:py-12">
      ...
      {props.origin === 'example' && props.taskCount > 0 ? <section className="mt-5 rounded-2xl border border-slate-700 bg-slate-900/60 p-5" aria-labelledby="launch-guide-heading">
        <h2 id="launch-guide-heading" className="text-lg font-bold text-white">{ja ? 'レビューの一連の流れを試す' : 'Try the full review loop'}</h2>
        <p className="mt-2 text-sm leading-6 text-slate-300">{ja ? '決定論的Preflightを実行し、JSON schemaの指摘を開き、Designで場所を表示して手動修正し、再評価して、所有できる形式でエクスポートします。' : 'Run deterministic Preflight, open the JSON schema finding, Locate it in Design, improve it manually, re-evaluate, then export the workflow you own.'}</p>
        <ol className="mt-3 list-decimal space-y-2 pl-5 text-sm text-slate-200">{(ja ? ['Preflightを実行', 'Readinessを開く', 'JSON schemaの指摘をLocate', 'Designで編集して再評価', 'Export'] : ['Run Preflight', 'Open Readiness', 'Locate the JSON schema finding', 'Edit in Design and re-evaluate', 'Export']).map((step) => <li key={step}>{step}</li>)}</ol>
      </section> : null}
      <p className="mt-5 text-xs leading-5 text-slate-400">GPT-6 Astra Challenge · September 18, 2026</p>
