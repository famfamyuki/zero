...
const completeEnv: Record<string, string> = {
  ...
  STRIPE_SECRET_KEY: 'sk_live_secret-test-value',
  ...
  ARCHITECTURE_REVIEW_STRIPE_MODE: 'live',
  ARCHITECTURE_REVIEW_PRICE_CURRENCY: 'usd',
  ARCHITECTURE_REVIEW_PRICE_UNIT_AMOUNT: '1200',
...
test('environment template inventories every readiness key, fixes quota at 10, and contains no secrets', () => {
  ...
  assert.match(template, /^ARCHITECTURE_REVIEW_PRICE_CURRENCY=usd$/m);
  assert.match(template, /^ARCHITECTURE_REVIEW_PRICE_UNIT_AMOUNT=1200$/m);
  assert.match(template, /^ARCHITECTURE_REVIEW_MODEL=gpt-5\.6-sol$/m);
  assert.match(template, /^ARCHITECTURE_REVIEW_MAX_PROVIDER_INPUT_BYTES=32768$/m);
  assert.match(template, /^ARCHITECTURE_REVIEW_MAX_OUTPUT_TOKENS=4096$/m);
  assert.match(template, /^ARCHITECTURE_REVIEW_MAX_WORST_CASE_COST_MICRO_USD=250000$/m);
  assert.doesNotMatch(template, /sk_(?:live|test)_|price_[A-Za-z0-9]|https:\/\/[^\s=]+/);
});
...
test('Stripe Test Mode is usable before live external approvals, while Production remains fail closed', () => {
  const testEnv: Record<string, string> = { ...completeEnv, ARCHITECTURE_REVIEW_STRIPE_MODE: 'test', STRIPE_SECRET_KEY: 'sk_test_secret-value' };
  for (const key of Object.keys(testEnv)) if (key.endsWith('_APPROVED')) testEnv[key] = 'false';
  assert.equal(inspectPaidArchitectureReviewReadiness(testEnv, { target: 'test' }).configurationReady, true);
...
test('invalid booleans, identifiers, public URLs, quota and cost bounds fail closed', () => {
  const cases: Array<[string, string, string]> = [
    ...
    ['ARCHITECTURE_REVIEW_STRIPE_MODE', 'sandbox', 'invalid_enum'],
    ['ARCHITECTURE_REVIEW_PRICE_CURRENCY', 'jpy', 'invalid_price_contract'],
    ['ARCHITECTURE_REVIEW_PRICE_UNIT_AMOUNT', '1199', 'invalid_price_contract'],
    ['ARCHITECTURE_REVIEW_STRIPE_TAX_ENABLED', 'false', 'invalid_boolean'],
    ...
    ['ARCHITECTURE_REVIEW_PROVIDER_BUDGET_HARD_CEILING_MICRO_USD', '49000000', 'invalid_budget_thresholds'],
  ];
  ...
  const modeMismatch = inspectPaidArchitectureReviewReadiness({ ...completeEnv, STRIPE_SECRET_KEY: 'sk_test_secret-value' });
  assert.ok(modeMismatch.issues.some((issue) => issue.key === 'STRIPE_SECRET_KEY' && issue.code === 'stripe_mode_mismatch'));
});
