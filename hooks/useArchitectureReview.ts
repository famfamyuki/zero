...
export function useArchitectureReview(graph:GraphData,preflight:Preflight,lang:Language){
  ...
  const [previewAccess,setPreviewAccess]=useState({enabled:false,allowed:false,userId:null as string|null});
  const previewTester=Boolean(session&&previewAccess.allowed&&previewAccess.userId===session.user.id);
  const accessMode=previewTester?'preview_tester_v0':'paid_subscription_v0';
  ...
  const loadAccess=useCallback(async()=>{
    const previewResponse=await fetch('/api/architecture-review/preview-access',{headers:authHeaders(),cache:'no-store'}).catch(()=>null);
    const preview=previewResponse?.ok?await previewResponse.json() as {enabled:boolean;allowed:boolean}:{enabled:false,allowed:false};
    setPreviewAccess({enabled:preview.enabled===true,allowed:preview.allowed===true,userId:session?.user.id??null});
    if(!session||preview.allowed===true){setAccess(null);return null;}
  ...
  useEffect(()=>{if(offer&&!previewAccess.enabled)trackEvent('paid_review_offer_shown',{offer_version:'0.1.0',access_state:session?(access?.state??'loading'):'signed_out'});},[offer,session,access?.state,previewAccess.enabled]);
  ...
  const run=useCallback(async()=>{if(!evidence||busy.current||!session||(!previewTester&&access?.state!=='active'&&access?.state!=='active_canceling'))return; busy.current=true; const previous=state.result; setState({status:'loading',result:previous,stale:Boolean(previous&&previous.workflowFingerprint!==fingerprint),errorCode:null}); trackEvent('architecture_review_requested',{review_version:ARCHITECTURE_REVIEW_RESULT_VERSION,evidence_version:ARCHITECTURE_REVIEW_EVIDENCE_VERSION,access_mode:accessMode});
    try{const response=await fetch('/api/architecture-review',{method:'POST',headers:{'Content-Type':'application/json',...authHeaders(),'Idempotency-Key':crypto.randomUUID()},body:JSON.stringify({version:'0.1.0',locale:lang,evidence})}); const payload=await response.json() as {result?:ArchitectureReviewResultV0;error?:ArchitectureReviewErrorCode}; if(!response.ok||!payload.result)throw Object.assign(new Error('review failed'),{code:payload.error??'provider_error'}); const result=payload.result; setDisplayEvidence(evidence); setState({status:'available',result,stale:result.workflowFingerprint!==currentFingerprint.current,errorCode:null}); trackEvent('architecture_review_completed',{review_version:ARCHITECTURE_REVIEW_RESULT_VERSION,evidence_version:ARCHITECTURE_REVIEW_EVIDENCE_VERSION,access_mode:accessMode});await loadAccess();}
    catch(error){const code=((error as {code?:ArchitectureReviewErrorCode}).code??'provider_error') as ArchitectureReviewErrorCode; setState({status:'error',result:previous,stale:Boolean(previous&&previous.workflowFingerprint!==currentFingerprint.current),errorCode:code}); trackEvent('architecture_review_failed',{review_version:ARCHITECTURE_REVIEW_RESULT_VERSION,error_code:code,access_mode:accessMode});await loadAccess();}finally{busy.current=false;}},[evidence,fingerprint,lang,state.result,session,access?.state,previewTester,accessMode,authHeaders,loadAccess]);
  const canRun=Boolean(session&&(previewTester||access?.state==='active'||access?.state==='active_canceling'));
  return{state,evidence,displayEvidence:state.result?displayEvidence:evidence,run,currentWorkflowFingerprint:fingerprint,paid:{previewEnabled:previewAccess.enabled,previewTester,session,authLoading,access,offer,busy:paidBusy,checkoutSyncing,message:paidMessage,canRun,signIn,signOut,checkout,manageBilling,refreshBilling}};
}
